...
- планета определяется суммой градусов географической долготы и широты местности.
...
- Повесьте в доме или на рабочем месте (ориентируясь по сторонам света - Север сверху)
- Украсьте сектора жилища соответствующими цветами, символами, лампами, благовониями или мантрами. Если в мандале преобладают цвета Раху, Кету или Сатурна, то следуйте рекомендациям по коррекции атмосферы жилища.
""")
...
if st.button("Построить мандалу"):
    try:
        ...
        if any(z in [4, 7, 8, 9] for z in [z1, z2, z3, z4]):
            ...
            st.markdown("**Рекомендуется коррекция через свет, чистоту, мантры и янтры, символы благотворных планет: Юпитер, Луна и Солнце, живой огонь, благовония и звуки. Не злоупотребляйте цветами Раху, Кету или Сатурна в интерьере, сбалансируйте их нейтральным песочным цветом и натуральными материалами: дерево, камень, лён.**")
...
### 🔢 Интерпретация чисел в вашей янтре:
""")

        used_zones = {z1, z2, z3, z4}
        for num in sorted(used_zones):
            name, color, meaning = planet_info[num]
            st.markdown(f"**{num} — {name}**: {meaning.capitalize()} (цвет — {color})")
...
🔍 **Для полной интерпретации и коррекций вы можете обратиться к астрологу по [этой ссылке](https://goroskop1008.ru/uslugi/#consult#!/tproduct/842449103-1607970659374)**
